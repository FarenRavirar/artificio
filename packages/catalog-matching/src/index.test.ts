...
describe('matchSystemNameExact', () => {
  const systems: MatchableSystemEntry[] = [
    { id: 'dd', name: 'Dungeons & Dragons', name_pt: 'Dungeons & Dragons', aliases: ['D&D'] },
    { id: 'cain', name: 'CAIN', name_pt: null, aliases: [] },
  ];

  it('matches by exact normalized name', () => {
    expect(matchSystemNameExact('dungeons and dragons', systems)?.id).toBe('dd');
  });

  it('matches by exact normalized alias', () => {
    expect(matchSystemNameExact('d&d', systems)?.id).toBe('dd');
  });

  it('does not match approximate/fuzzy input — exact layer only', () => {
    expect(matchSystemNameExact('Dungeons and Dragon', systems)).toBeNull();
    expect(matchSystemNameExact('CAIN 1.3', systems)).toBeNull();
  });

  it('returns null for null/undefined/empty input', () => {
    expect(matchSystemNameExact(null, systems)).toBeNull();
    expect(matchSystemNameExact(undefined, systems)).toBeNull();
    expect(matchSystemNameExact('', systems)).toBeNull();
  });
});
