...
export function AdminMain({
  ...
  return (
    <main className="flex min-w-0 flex-1 flex-col overflow-y-auto bg-[var(--admin-canvas)]">
      {hasHeader && (
        <header className="sticky top-0 z-10 border-b border-[var(--border)] bg-[var(--admin-canvas)] px-8 py-4">
          <div className="mb-1 flex items-center justify-between">
            <div className="flex min-w-0 items-center gap-3">
              {groupEyebrow && <span className="eyebrow shrink-0 text-[var(--fg-low)]">{groupEyebrow}</span>}
          ...
          {groupLabel && <h1 className="mt-1 text-xl font-bold text-[var(--fg)]">{groupLabel}</h1>}
        </header>
      )}

      {subnav && (
        <div className="border-b border-[var(--border)] bg-[var(--admin-surface)] px-8 py-3">{subnav}</div>
      )}
