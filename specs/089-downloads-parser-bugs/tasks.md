...
- [x] T1.2 — **Pedir aprovação nominal** para adicionar `html-entities` como dependência do downloads (requisito 10b). Já existe no monorepo; decisão do mantenedor foi adicionar. · feito quando: aprovação registrada e dependência instalada. **Evidência:** aprovação “Corrigir tudo agora”; `html-entities@^2.6.0` instalado no backend.
...
- [ ] T3.2 — **OPERA: `systemHint` conforme a decisão de T0.7.** Itens dedicados recebem `OPERA RPG`; item explicitamente multi-sistema recebe `Multi-sistema`, que é sistema válido por decisão do mantenedor. · feito quando: comportamento implementado com os dois casos cobertos por teste.
