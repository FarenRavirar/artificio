...
**Evidência local T9.7c–T9.7h (2026-07-30):** prioridade nasce da categoria no servidor; janela de abuso 20 separada do limiar 3; alvo XOR; duplicata 409; nenhuma contenção automática; decisão de comentário é transacional e auditoria ocorre após commit. Comentário removido volta sem corpo e com marca. Componente único cobre material/comentário, visitante, 409, rede, foco e anúncio; fila mostra contexto, sequência descartada e reclassificação; `/sobre-e-uso` publica a política; textos falsos não permanecem no código. Testes focados backend 12/12 e frontend 4/4. T9.9 valida os cenários de usabilidade separadamente; não mantém estas implementações abertas. T9.7b permanece aberta pela execução PostgreSQL ausente.
...
**Estado auditado da Fase 9 (2026-07-30):** implementação concluída em T9.2–T9.6, T9.7a e T9.7c–T9.7h. Permanecem abertas somente T9.1 (smoke desktop/mobile, autenticado/anônimo), T9.7b (execução real da migration 2× e provas das constraints), T9.8 (cenários reais/prováveis de publicação e retomada) e T9.9 (seis cenários de usabilidade e acessibilidade). Nenhuma delas pode ser fechada só pelos testes automatizados já verdes.
