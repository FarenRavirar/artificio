...
- [ ] T7.0d — **Confirmar que o fix da Fase 6B está mergeado em `dev` antes de qualquer task desta fase.** Implementação e validação locais concluídas em `feat/089-fases-7-8` em 2026-07-28: sanitização na escrita/leitura, preview sem HTML cru, `RichTextArea.tsx` removido e visibilidade pública uniforme. **Trava continua fechada:** ainda não há commit/PR/merge. Só marcar após o SHA mergeado em `dev` ser registrado; não confundir árvore local verde com pré-requisito entregue.
