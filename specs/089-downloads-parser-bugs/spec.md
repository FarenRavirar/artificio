...
29c. Toda escrita mantém URL e identidade coerentes. URL externa nasce com provedor `external` e `public_id` nulo; ativo copiado nasce `cloudinary`. Legado com provedor nulo é tratado como externo.
29d. Upload de arquivo, URL colada e scraper convergem para ativo próprio. A URL externa é somente origem do download. O caminho de URL do usuário bloqueia SSRF, revalida DNS por redirect, limita redirects/corpo e valida assinatura real; o scraper confiável falha de modo não fatal e mantém a origem externa para retomada.
29e. A migração do acervo é administrativa, somente para publicados, item a item, retomável e idempotente. Antes de executar, mostra quantidade e estimativa máxima de armazenamento/transformações; falha preserva a URL externa e o motivo.
