...
| 20 | auditoria changelog | `glossario/database/changelogs.json` | ⚠️ cosmético | Frações de segundo inconsistentes (`.888791`, `.4246`, `.002405`) e fuso misturado (`+00:00` vs `-03:00`) — herança da migração DB. | **procede** | **Investigado 2026-06-21:** 4 entradas com frações de segundo (4-6 dígitos), 13 entradas com `+00:00` (UTC) enquanto a nova entrada usa `-03:00` (Brasília). Normalizado: frações removidas, todas as 13 entradas antigas convertidas para `-03:00` (sem alterar hora — eram wall-clock Brasileiro, não UTC real). | commit `(pendente)` |
| 21 | auditoria changelog | `glossario/frontend/ChangelogModal.tsx:37-41` | ⚠️ cosmético | Sem `AbortController` no fetch — fetch completa mesmo após unmount (React 18+ ignora setState, mas é menos robusto). | **procede** | **Investigado 2026-06-21:** Padrão do mesas (já corrigido com signal) replicado no glossario: `useEffect` cria `AbortController`, `api.get` recebe `{ signal }`, cleanup chama `controller.abort()`. Retry separado com `useCallback` (sem abort — ação explícita do usuário). | commit `(pendente)` |
| 22 | auditoria changelog | `links/LinksHeader.tsx:23` | ⚠️ cosmético | `variant` não passado ao `Header` — logo não alterna navy/negativo com o tema (sempre navy). | **procede** | **Investigado 2026-06-21:** Todos os outros apps passam `variant`. Links não passava. `useTheme()` de `@artificio/ui` usa `useSyncExternalStore` que quebra SSR do Astro (`Missing getServerSnapshot`). Fix: `useEffect` + `useState<Theme>` + `MutationObserver("data-theme")` inline no LinksHeader — lê tema do DOM pós-mount (client:load), sem hook SSR-incompatível. | commit `(pendente)` |
| 20 | deploy beta (CI) | `apps/site/server/server.ts:11` + `apps/site/package.json` | 🛑 crash-prod | **Deploy beta falhou**: `site-beta-app não ficou healthy`. Log: `Error [ERR_MODULE_NOT_FOUND]: Cannot find package '@artificio/config' imported from /repo/apps/site/server/server.ts`. `tsx server/server.ts` morre no boot → container nunca healthy → deploy aborta. | **procede** | **Investigado 2026-06-21:** Mesma classe do #5 (links), mas em `site` e NÃO capturada pelos bots. 041 (commit `ba2b647`) adicionou `import { BRAND_ORIGIN } from "@artificio/config"` (`server.ts:11`, uso `:42`) sem declarar a dep. `@artificio/config` é workspace package: resolvido por hoist/symlink em dev, mas o container de produção poda deps (install isolado) → não linka → `MODULE_NOT_FOUND` no startup. Fix: `"@artificio/config": "workspace:*"` em `apps/site/package.json` deps + `pnpm install` (lockfile). Lição: bots de PR não exercitam o boot do container — gap de cobertura registrado em `tasks-2.md` (D-041-15). | commit `(pendente)` |
