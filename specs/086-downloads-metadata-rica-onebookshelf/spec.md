...
25. **`material_type` deixa de ser texto livre** — hoje é `z.string().trim()` sem enum, preenchido por `<input>` de texto (gap 10), o que produz valores duplicados por grafia ("aventura"/"Aventura") e poluiria a faceta do requisito 23. Toca backend (validação) e frontend (formulário), então **é resolvido nesta spec**, não empurrado pra backlog — regra de débito acima. A **forma** é decisão do mantenedor, a perguntar antes de codar: enum fechado em Zod + `<select>` (mais rígido; exige definir a lista e migrar os valores já gravados) ou normalização + taxonomia consultável como sistemas/edições (mais flexível). A fase não fecha deixando o campo livre.
