...
56f. **Validação de payload que não chega ao uso é validação decorativa — achado de review da PR #218 (CodeRabbit, 2026-07-27).** A rota `/resolve` validava o corpo com Zod (`.trim()`, `.max(200)`) e em seguida entregava **`req.body` cru** aos resolvers, que liam o valor original por `readTrimmed(ctx.body.name)`. O schema barrava o payload inválido, mas o payload **válido** seguia sem normalização: uma string com espaços sobrando, ou no limite do tamanho, chegava a `createCatalogMaterialType` exatamente como veio. Corrigido passando `parsed.data` — `looseObject` preserva as chaves extras que cada resolver consome, então o que muda é apenas que os campos declarados chegam normalizados.
