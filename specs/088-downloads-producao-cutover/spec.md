...
56d. **Escrita no catálogo central atravessa fronteira de serviço, e isso muda dois requisitos de robustez — 2ª rodada de review da PR #218 (Codex, 2026-07-27).** O catálogo vive no banco do **Site**; a transação do Downloads não o alcança. Dois defeitos reais decorriam disso:
   ...
   > **Débito conhecido, herdado e não corrigido aqui:** o fluxo de **sistema** tem o mesmo defeito de atomicidade — `replaceAliases` (`apps/site/db/repo/catalog.ts`) faz `DELETE` + `INSERT`, e `addCatalogNodeAlias` faz o mesmo read-modify-write. Já está em produção desde a spec 086. Não é regressão desta spec, e corrigi-lo exige tocar o contrato que `mesas` e `glossario` também consomem.
