...
| 083 | downloads rejeição motivo legal + e-mail | **F0-F7 implementados localmente (2026-07-23).** Reprovação exige categoria estruturada (enum configurável via admin, com enquadramento legal BR) + e-mail transacional (Resend) para reject/approve, orquestrado com retry + log auditável (`download_email_log`). Rota interna nova em `accounts.` reusa `SERVICE_SECRET` já existente (WS3). Testes novos verdes (accounts 13/13, downloads-backend 93/93, `@artificio/email` 4/4), lint/tsc/build/`verify:api` limpos. Sem commit — aguardando autorização nominal (toca SSO). |
| 084 | downloads scraper de indexação automática de terceiros + regra pétrea de só-português | **Investigação/spec fechada (2026-07-24), zero código.** Scraper de 8 fontes (itch.io, DriveThruRPG, DMs Guild, RPG Grátis, Grimórios & Dados, OPERA RPG, Catarse, Newton Rocha — RedeRPG fora do MVP por exigir login) indexa direto (sem revisão humana) itens grátis/PWYW **e em português** como `access_kind=external_link`. D118 reescreve o modelo de automação de Downloads (dois atores: humano moderado + scraper autônomo restrito); D090/D095/D100 marcadas como revisadas com a cláusula exata. D119 (nova): catálogo é só português, schema/validação/scraper/frontend/SEO/página institucional — auditoria confirmou gap total. Cron diário automático pras fontes sem anti-bot; DriveThruRPG/DMs Guild (WAF confirmado, 403 até em robots.txt) seguem só disparo manual, escalando Modo 1 (fetch) → 2a (`patchright`) → 2b (`camoufox`) → 3 (script local fora da IA, Node/Python standalone com profile Chrome real do mantenedor — não `claude-in-chrome` em loop, custo de IA proibitivo pra scraping repetitivo). Detecção de idioma via `franc-min` + DeepSeek (spec 052) como desempate pontual; itch.io usa filtro nativo `lang-pt-BR`. Achado de segurança: repo typosquat identificado (`jo-inc/camofox-browser`) e não usado. Débitos abertos: lacuna jurídica OGL/ORC, risco de suspensão de conta pessoal no Modo 3, detecção cobre só metadado (não arquivo), nenhum engine anti-bot dá garantia real, RedeRPG fora do MVP. |
