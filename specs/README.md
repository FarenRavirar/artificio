...
| 054 | reorganização da `/gestao` (sidebar + IA + renomeações) | **planejada (2026-06-27) — PRIORITÁRIA / GATE DE BLOQUEIO.** Abas-topo → sidebar persistente; 7 grupos (subseções detalhadas); rotas aninhadas; renomeações **ampliadas a identificadores** (rótulo+enum/rota; persistido só via migration); padrão de botões; moderação unificada (reaproveita 049/DEB-048-34). SDD Completo (mesas FE + possível `packages/ui` + backend onde rename tocar contrato). **GATE: 054 → 053 → 052.** 053 Frente A (mesma tela) bloqueada até 054 fechar. |
