...
- [ ] B8 — **Dinamismo/animação da logo do Header** (`packages/ui`) · pedido do mantenedor 2026-06-13. Hoje a logo é `<img>` estática; falta efeito/animação (hover, micro-interação ou logo dinâmica). Precisa direção de design antes de implementar (estilo/intensidade). Deformação no mobile já corrigida à parte (`object-fit:contain`+`flex-shrink:0`).
