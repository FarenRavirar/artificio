...
| `apps/accounts/src/serviceCredential.ts` + `requireServiceCredential.ts` | T2.2a — resolução de credencial por `source_app`/`realm`/escopo, devolvendo **identidade** em vez de `boolean`; substitui o `SERVICE_SECRET` global de `serviceToken.ts` nas rotas internas. Operacional de emissão/rotação em `docs/agents/deploy-runbook.md` §Credenciais de serviço |
