...
- [x] T6.4 — **Arquitetura de runtime do comentário no `site`** (requisito 25). A versão anterior dizia só "adotar o pacote", sem dizer onde ele roda — e o blog é **SSG** (`astro.config.mjs:7`), com os posts gerados por `getStaticPaths` (`pages/blog/[slug].astro:7`); o servidor só consulta comentários para um contador administrativo (`server/server.ts:116`). Definir: fachada Express **same-origin**; validação de post publicado; `subject_id = String(post.id)`; `canonical_path = /blog/${slug}/`; **ilha React abaixo do artigo, `client:visible`** (adia o JavaScript até entrar no viewport, sem hidratar a página toda); lista degradável e escrita SSO backend-to-backend. · feito quando: a página estática continua estática e a ilha carrega só quando visível.
  ...
  **Host da conversa EXTRAÍDO para o pacote (`packages/comments/src/useConversationHost.tsx`), com autorização do mantenedor.** O Sonar acusou **72,9% de duplicação** em `useSiteConversation.ts`; a medição confirmou 174 linhas divergentes de 261, com diferença real limitada a `subjectType`, `sourceApp`, caminho da fachada e `baseUrl`. **A duplicação não era o problema — era o sintoma:** as duas correções de review aplicadas no `site` (`reload` assíncrono que descarta páginas mescladas, e `setPages` funcional) **nunca chegaram ao `downloads`**, que estava com os dois defeitos em produção — "Comentário publicado." antes da árvore nova chegar, e duas páginas de `more` concorrentes descartando uma à outra. Adotar o host compartilhado corrige os dois sem edição própria. Resultado: os hosts caíram de 261/265 para **68/89 linhas**, com 20 linhas idênticas restantes (assinatura da interface e chamada do hook — extrair mais inventaria abstração). O `mesas` herda na Fase 7 sem uma terceira cópia.
  **Updater de `setPages` deixou de fazer trabalho impuro (achado de review, PR #264).** A versão da extração chamava `mergeCommentsThreadPage` **dentro** do updater e sinalizava falha mutando variável de escopo externo — as duas coisas quebram o contrato de pureza que o React exige ali, e não é teórico: `StrictMode` está ativo (`downloads/src/main.tsx:30`) e invoca o updater **duas vezes**, então a mesclagem rodava duplicada a cada página de `more`. Agora a mesclagem acontece fora, contra uma ref que espelha o estado; a ref resolve o que o updater funcional resolvia (ler o valor mais recente, para que duas páginas concorrentes se acumulem em vez de uma descartar a outra) sem lógica dentro dele. `reload` zera ref e estado juntos — ref não limpa faria a próxima mesclagem partir da árvore que a recarga invalidou. **O `downloads` não precisou de edição própria:** já delega ao host compartilhado desde a extração, então herda a correção. `Content-Type` passou a ser declarado só quando há corpo (declará-lo em `GET`/`DELETE` é o que torna requisição simples preflighted em CORS — irrelevante no `site` same-origin, não no `downloads`); 3 testes novos do transporte, com mutação confirmando.
  **Débitos fechados em 2026-08-16, por pedido do mantenedor.** (1) **O `downloads` tinha o mesmo vazamento de leitura do `site`, e estava em produção:** `GET /api/v1/community/conversation` não passava pelo guard, então `?subject_id=<material em rascunho>` confirmava a existência pela diferença entre `200` e `404`. Corrigido com o guard na leitura — aqui, diferente do `site`, ele distingue quem pergunta (o criador enxerga o próprio rascunho, `visibleOnlyToActor`). Teste novo + mutação confirmando; o `beforeEach` da suíte passou a autorizar por padrão, porque a leitura agora consulta o guard. (2) **Ilha React do `site` ganhou teste** (`PostConversation.test.ts`, 6 casos) sobre `permissionsFor`, exportada para isso — é onde a decisão mora, e exercitá-la direto dispensa `jsdom`, que não é dependência do `site`. Cobre a decisão 23 (legado respondível mas imutável) e a distinção §4 entre `removed` e `pending_review_hidden`. Mutação: permitir voto em legado derruba. (3) **`links` saiu do `echo lint TODO`** — `eslint.config.js` criado no molde do `site`, um import morto removido (`configure` em `server/lib/cloudinary.ts`), script trocado para `eslint .` **depois** do verde. (4) **`site-admin` ficou aberto de propósito:** config criada e `eslint-plugin-react-hooks` instalado com autorização nominal, mas a primeira execução achou 6 erros + 4 avisos reais (5× `setState` síncrono em efeito, 1× ref durante render) — corrigir é refactor do editor de conteúdo, sem suíte no app. Trocar o script antes do verde violaria a trava do `AGENTS.md`; registrado em `sessoes/26-08-16_1_site-admin_lint-react-hooks.md` para branch própria.
  **Alerta do CodeQL sobre `optionalAuth.test.ts` era falso positivo, e foi eliminado mesmo assim:** o handler sinalizado é o servidor efêmero do teste; em produção `optionalAuth` só existe atrás de `readRateLimiter` (`community-api.ts:268`). O limiter passou a ser montado também no duplo — custa nada e mantém o teste fiel à composição real.
  **Comportamento de estado do host agora tem teste (`useConversationHostState.test.tsx`, 4 casos).** Antes só o transporte era coberto — o que a requisição leva —, e não o que fica na tela depois, que é justamente onde os dois achados de review moravam. Usa `act` + `createRoot`, o padrão do próprio pacote (`CommentsConversation.test.tsx`), **sem adicionar `@testing-library/react`**: 7 pacotes do monorepo a declaram, mas o `comments` não, e dependência nova é decisão do mantenedor. **Um dos quatro estava errado e a mutação revelou:** o teste de "`reload` descarta a página mesclada" usava revisão diferente na recarga, onde o `effectiveState` já descartaria sozinho — passava mesmo com `setPages(null)` removido. Corrigido para revisão igual (o caso real: moderação oculta um comentário sem mudar a revisão), e aí a mutação derruba.
  **Três achados do Sonar corrigidos.** `replace(/\\/g)` → `replaceAll`; cadeia de promises do exportador → `try/catch/finally` com top-level await (o `catch` do `close` é silencioso de propósito: falha ao fechar não pode mascarar o `exitCode` que a falha real definiu); e complexidade cognitiva 21 → limite 15 em `importLegacyComments`, extraindo a resolução do pai para `resolveParent`. A sentinela `PAI_AUSENTE` é `Symbol`, não `null`, porque `null` já significa "este comentário é raiz" — um valor para os dois casos faria o filho órfão ser importado como raiz, que é exatamente o que a task proíbe. Revalidado contra PostgreSQL real depois da refatoração: 21/21, e os 25 comentários reais reimportados com `inserted: 25` / `skipped: 25` e árvore íntegra.
  **`.tsx` num arquivo sem JSX, de propósito:** `packageBoundary.test.ts:52` varre todo `.ts` da raiz de `src` (exceto `react.ts`) e recusa `import … from 'react'` — a trava do requisito 21b que mantém o root utilizável em backend e SSR. A suíte pegou o arquivo na primeira tentativa; todo código React do pacote usa `.tsx` por essa razão, e o motivo ficou registrado no cabeçalho para não ser renomeado de volta.
  **Segunda rodada de review na PR #264 — dois vazamentos e três erros de robustez, todos corrigidos.** (1) **A LEITURA não passava pelo guard**, só a escrita: `?subject_id=<id de rascunho>` distinguia post existente de inexistente pela resposta, um oráculo de existência sobre conteúdo não publicado — vale mesmo com árvore vazia, porque o que vaza é o id ser válido. T6.4 pede "validação de post publicado" sem restringir a escritas, e `contrato-http-v1.md:602` lista leitura como dever da fachada. O `downloads` tem o mesmo desenho (`communityComments.ts` só chama `authorizeSubject` nas escritas) — **débito lá, reportado ao mantenedor**. (2) **`Idempotency-Key` não era validado antes de virar header:** `fetch` lança `TypeError` em valor não-ASCII e o `catch` traduzia em `503`, dizendo "o `accounts.` está fora" quando o problema era o header do cliente. (3) **`subject_id` de 19 dígitos passava no regex mas estoura o `BIGINT`** (`9999999999999999999 > 9223372036854775807`), virando erro de driver — `500` onde o certo é `404`. (4) **O lookup do pai filtrava `legacy_source` por `source_app`**, campo diferente do que a linha grava (`comment.legacy_source`): quando divergem, o pai não é achado e a árvore achata em silêncio. (5) **`reload` era síncrono e não descartava as páginas mescladas** — o pacote aguarda o retorno antes de anunciar sucesso, então o "Comentário publicado." aparecia antes da árvore nova, e a página do `more` (retrato da revisão anterior) escondia o comentário recém-escrito.
  **Uma asserção minha não provava nada:** `expect(linhaRaiz.root_id).toBe(linhaRaiz.parent_id === null ? linhaRaiz.root_id : null)` compara `root_id` consigo mesmo — passa sempre, inclusive com a hierarquia achatada. Trocada por comparação contra o `id` da própria linha. Mutação confirma: achatar a árvore agora derruba 2 testes.
  **Revalidado com os 25 comentários reais** depois das correções: `inserted: 25` / reexecução `skipped: 25`, 22 raízes + 3 respostas, `raizes com root_id = id: true`, `filhos herdam root_id do pai: true`.
  **Achado do CodeQL na PR #264 — a fachada não tinha rate limit próprio, e isso era defeito de produção, não do teste.** O alerta apontou o arquivo de teste, mas a causa estava no `community-api.ts`: as 6 rotas contavam só no `globalLimiter` do servidor (`server.ts:36`, 300/min), que soma **todas** as rotas do `site` num balde único — rajada de escrita de comentário consumiria o orçamento da navegação do blog, e escrita ficaria com o mesmo teto da leitura apesar do custo e do risco de abuso maiores. A fachada equivalente do `downloads` já aplicava `readRateLimiter`/`writeRateLimiter` por rota (`communityComments.ts:214-287`); replicados aqui com os mesmos números (300 e 60 por 15min), para que o mesmo abuso encontre a mesma resposta nos dois módulos. Teste sobre o fonte garante que nenhuma rota nova fique descoberta; mutação (remover o limiter do `GET`) faz falhar.
  **Os outros dois alertas eram do teste e procediam:** `String(url).startsWith('https://accounts.example')` casaria também com `https://accounts.example.evil.com`. Trocado por comparação de `URL.hostname`. O efeito aqui seria só um teste passando por engano, mas é o mesmo padrão que em produção vira SSRF/bypass de allowlist — corrigido para não virar molde copiado adiante.
