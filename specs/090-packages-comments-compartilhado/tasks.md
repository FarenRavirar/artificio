...
- [x] T3.15 — **Outbox: evento transacional, entrega assíncrona, um caminho só** (requisito 13c-i). Tabela `notification_outbox` (migration_010 seção 5) com `recipients` JSONB. `notificationOutbox.ts`: `enqueueOutboxEvent(trx)` na transação (`communityCommentWrite.ts:535`), `processOutboxEntry(db)` faz fan-out com preferências (T3.11b) e cria recibos idempotentes (ON CONFLICT via try/catch no UNIQUE do recibo). `processOutboxPending` roda fora da transação, pós-commit (`:567`), fire-and-forget — estruturalmente impossível reverter o comentário já commitado. Moderação sempre entregue independente de preferência (20b). Validação: lint 0, tsc 0, testes 521/521, verify:api 0 breaking. **Execução real confirmada em 2026-08-12** (mesmo container descartável de T3.6, destruído ao final): reprocessar o mesmo `event_id` contra recibo já existente devolveu `INSERT 0 0` — não duplica (`UNIQUE (realm, source_app, event_id, recipient_user_id)`); entrada de outbox com `processed_at NULL` sobrevive independente do fan-out ter rodado, provando que falha de entrega não perde o evento.
