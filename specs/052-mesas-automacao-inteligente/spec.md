...
- **R7 — IA auxiliar para extração ambígua.** Quando o parser determinístico tem baixa confiança, uma camada IA propõe campos (sistema, vagas, dia/hora, contato), sempre como sugestão revisável, validada por schema (Zod) com timeout/retry. **Decisão D087 (2026-06-30):** provider primário = **DeepSeek**, atrás de interface adapter `AiFieldSuggester` (`suggest(payload)→campos+score`) que permite trocar/empilhar providers ("todos") sem mexer no pipeline; chave em segredo (`.env` gitignored). Gatilho = `confidence < threshold` por campo; IA nunca é fundação; fallback ao parser determinístico + **kill switch** (R14). **Privacidade não é restrição aqui** (canal Discord público → conteúdo vai a site público; nomes/IDs/@menções já são públicos): minimização do payload mantida só por **economia de token**, e segredos (token/cookie do exporter) nunca entram no payload.
- **R8 — Aproveitar histórico da Fase G.** Correções humanas registradas (antes/depois) viram exemplos few-shot, conjunto de avaliação e base de comparação modelo/prompt. **Decisão D087:** o aprendizado vive num **learning-store = tabela nova no schema mesas** (ex. `discord_field_learning`, migration isolada), keyed por `campo + token normalizado` (ex. `sistema:"d&d 5e"→"D&D 5.2"`). **Fluxo em camadas (economia de token):** parser → se baixa confiança, lookup no store (custo zero token) → *cache hit* usa correção passada SEM chamar IA → *cache miss* chama IA com few-shot dos N exemplos mais próximos do store → a revisão humana vira nova entrada no store. Cada erro corrigido uma vez não volta a gastar token de IA.
