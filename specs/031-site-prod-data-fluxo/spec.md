...
- **R4** — Beta sincroniza dados do prod. Mecanismo: truncate + restore manual idempotente (trunca todas as tabelas + `pg_dump site-prod-db --data-only --exclude-table-data=schema_migrations | psql site-beta-db`, com `session_replication_role=replica`) executado antes de cada deploy do beta (opção A do plan.md). Automação contínua = spec futura. · feito quando: beta tem dados iguais ao prod após sync; editar no beta não afeta prod.
