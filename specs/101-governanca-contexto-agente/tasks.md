...
- [ ] F0.5.5 — **Verificar que dispara.** Para cada skill reescrita, observar ativação numa tarefa real que deveria acioná-la. Gatilho de skill é **probabilístico** — descrição boa aumenta a chance, não garante. Skill que não dispara não recebe regra crítica. · feito quando: o disparo foi observado, ou a skill está marcada como não confiável. **Parcial em 2026-09-10 (F5.1):** medido que as **22/22** do repo são de fato *carregadas* pelo OpenCode (`opencode debug skill`), depois de removidas as duas cópias globais que venciam a versionada. Carregamento é pré-condição do disparo, **não é o disparo** — a task continua aberta pelo que ela pede de fato: observar ativação em tarefa real.
...
- [ ] F5.1 — Rodar uma sessão real de trabalho de código **em cada um dos três harnesses** (Claude Code, Codex, OpenCode) — validar em um só não valida o mecanismo (H10). Testar também a hipótese do *gênio literal* (H11): o Codex reage diferente do Claude à redução de imperativos? Observando: o agente sentiu falta de algo movido? Os gatilhos dispararam quando deveriam? Algum hook reprovou indevidamente?
  - [x] **Claude Code — FEITO em 2026-09-10. Os 5 hooks funcionam em sessão real; zero falso-positivo.** Suítes isoladas **125/125** (`rtk-enforce` 36, `autorizacao-gate` 56, `rtk-read-gate` 13, `deploy-contract-gate` 11, `git-commit-msg-gate` 9). Trabalho de código real: `rtk pnpm vitest run src/community/materialSubjectGuard.test.ts` → **9/9**. `deploy-contract-gate`: **4/4** famílias cobradas, `index.ts` comum passou. `rtk-read-gate`: bloqueou `pnpm-lock.yaml`, liberou `AGENTS.md`, `package.json` e leitura com `offset`/`limit`. **Gatilho disparou sem ser provocado:** o `rtk-enforce` bloqueou um `sed -n '125,150p'` espontâneo e devolveu a forma correta, reemitida no mesmo turno — o comportamento que a F4.3 desenhou. **Nenhuma regra movida para skill fez falta** na sessão inteira.
  - [x] **Método corrigido — testar hook isolado NÃO valida o mecanismo.** Primeiro diagnóstico desta task foi errado: hooks chamados isoladamente deixavam `git status`/`rg`/`cat` passarem, o que parecia lacuna do `rtk-enforce`. Não é — o cabeçalho do próprio arquivo (`rtk-enforce.js:23-24`) diz que ele roda **depois** do `rtk hook claude` e só vê o que a camada 1 não reescreveu. Medido: `rtk hook check "git status"` → `rtk git status`; `"cat AGENTS.md"` → `rtk read AGENTS.md`. **Validação correta é das duas camadas juntas:** dos 5 comandos que a camada 1 devolve como `No rewrite`, **5/5** foram bloqueados pela camada 2 pelo hook certo (`pnpm test`, `pnpm verify:api`, `pnpm --filter mesas test` → `rtk-enforce`; `sudo apt-get install jq`, `ssh faren docker restart` → `autorizacao-gate`). Quem levar esta task ao Codex/OpenCode precisa testar a cadeia inteira, senão o resultado engana na mesma direção.
  - [x] **Caminho não-interativo descoberto para os outros dois harnesses.** `codex exec` (com `-s read-only`) e `opencode run` executam por linha de comando; melhor ainda, **`codex debug prompt-input`** renderiza o prompt visível ao modelo em JSON e **`opencode debug skill` / `debug agent`** inspecionam gatilhos **sem acionar o agente e sem consumir API**. Torna a F5.1 repetível, não teste manual de uma vez só. A autorização nominal que a §Autorização exige para acionar outro agente **não é necessária** para esses subcomandos de inspeção — são read-only.
  - [x] **Bug real encontrado e corrigido: cópia global de skill vencia a versionada.** `opencode debug skill --log-level DEBUG` registrou `WARN "duplicate skill name" name=duplicate-code-detector existing=~/.claude/skills/… duplicate=C:\projetos\artificio\.agents\skills\…` — a global ganhava e a do repo era **descartada**. Efeito: o OpenCode carregava **21 das 22** skills do repo, e a `duplicate-code-detector` em uso era a de **8.565 bytes**, versão anterior à correção da F0.5.3 — ou seja, **o trabalho da F0.5.3 não chegava ao agente**. Havia **três** cópias: repo (7.762 B, correta), `~/.claude/skills/` e `~/.opencode/skills/` (8.565 B cada, obsoletas). As duas globais removidas com autorização nominal do mantenedor, backup em `artificiobackup/skills-2026-09-10/{claude,opencode}-global-duplicate-code-detector/` (3 e 2 arquivos, bytes conferidos). **Medição depois:** repo **21 → 22/22** carregadas, `duplicate-code-detector` vem de `C:\projetos\artificio\.agents\skills\`, e **0** warnings de duplicata envolvendo o repo (restam 12 entre pastas globais, com nomes que o repo não usa — fora de escopo).
  - [ ] **Codex e OpenCode — pendente.** Depende de decisão do mantenedor sobre rodar `codex exec`/`opencode run` de fato (consome API dele).
...
- [x] F5.3 — **Decidido pelo mantenedor em 2026-09-10: opção (b), elevar o cap.** `project_doc_max_bytes = 131072` gravado em `~/.codex/config.toml` (chave top-level, ausente até então — o valor vinha do default do binário). Validado: TOML parseia (`tomllib.load`), chave lida como `int`, `AGENTS.md` = 69.496 bytes cabe com 61.576 de folga. Escolhido 128 KiB e não o valor justo porque colar em 69.496 faria o próximo parágrafo de governança voltar a truncar em silêncio — o modo de falha que esta spec existe para matar. Backup: `config.toml.bak-101-f53`. **Reverte D6** ("sem analgésico"), por decisão explícita dele. **Medido em 2026-09-10 (F5.1), e não exigiu sessão nova:** `codex debug prompt-input "teste"` renderiza o prompt visível ao modelo em JSON, sem consumir API. O `AGENTS.md` aparece **íntegro** (contido no prompt = `True`, última linha inclusive), o maior bloco tem 72.331 bytes, e há **0 ocorrências** de `truncat` ou `exceeds remaining budget` no JSON inteiro. Folga: 131.072 − 69.021 = **62.051**. *Correção de fato:* o arquivo hoje tem **69.021 bytes**, não os 69.496 citados acima — encolheu nos commits posteriores. **Achado que a decisão expôs:** o cap do Codex não era o único. Ver A8.3.
