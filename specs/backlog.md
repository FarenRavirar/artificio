...
| BL-QA-SITE-IMAGES | aberto | `specs/025-quality-lighthouse-program/lighthouse-findings.md`, `specs/025-quality-lighthouse-program/baseline-2026-06-16.md` | `apps/site`, Cloudinary, cards/posts/footer | reduzir transferencia de imagens (~472 KiB no anexo), adicionar dimensoes/srcset/sizes/prioridade sem quebrar zero-JS; baseline limpo melhorou para perf 85 mobile/96 desktop, entao nao e maior risco imediato | abrir fatia site imagens apos CLS/perf glossario; validar build + Lighthouse limpo |
| BL-QA-SHELL-CLS | local | `specs/025-quality-lighthouse-program/lighthouse-findings.md`, `specs/025-quality-lighthouse-program/baseline-2026-06-16.md`, `sessoes/26-06-16_3_quality_glossario-cls.md`, `specs/020` B13 | `apps/glossario` local; futuro `packages/ui`, shells Astro/React | baseline limpo confirmou `glossariobeta` CLS 0.647 mobile e 0.251 desktop; fatia local no glossario mantem badge/landing no primeiro render e fixa dimensao do footer logo; Lighthouse local mobile caiu para CLS 0.0002 | validar em `glossariobeta` apos deploy; se confirmar, decidir promover regra do footer logo para `packages/ui` por SDD Completo |
| BL-QA-GLOSSARIO-PERF | aberto | `specs/025-quality-lighthouse-program/lighthouse-findings.md`, `specs/025-quality-lighthouse-program/baseline-2026-06-16.md` | `apps/glossario` frontend/backend | baseline limpo confirmou perf 12 mobile, LCP 13.7s, TBT 1065ms; reduzir bundle inicial e tirar `/api/terms` do caminho critico; confirmar/remover Google Fonts residual se real | abrir spec perf glossario com testes de busca/termos/auth e Lighthouse limpo |
...
| BL-QA-ROBOTS-SEO | aberto | `specs/025-quality-lighthouse-program/lighthouse-findings.md` | `packages/content`, hosts de apps, nginx se necessario | validar/corrigir `robots.txt` invalido em glossario/mesas; confirmar content-type, conteudo bruto e ausencia de SPA fallback | rodar `curl -i` + validador; criar fix por host/helper |
