...
| BL-CDX-310 | local (F5) | `specs/026` F5, `sessoes/26-06-16_14`, `project-state.md` | CI/CD accounts, `apps/accounts/docker-compose.prod.yml` | IMPLEMENTADO LOCAL: compose prod versionado (healthcheck + env `${VAR}` + project/volume preservados), 4a entrada manifesto PROD-only (D042 sem accountsbeta), guard env=beta, `deploy-accounts.yml` deletado. accounts migra in-container (runner no-op) | aprovacao: commit/push/PR -> dry-run VM (build no clone) -> deploy prod via dispatch (`module=accounts mode=deploy`) -> smoke login/me/logout; rollback = revert + re-dispatch |
| BL-INFRA-CI-EFFICIENCY-F11 | local | `specs/026` F11, runs `27635199357`/`27635198756` | `_deploy-module.yml` CI, `deploy-mesas.yml` | IMPLEMENTADO local: CI cacheia pnpm store + turbo; mesas auto-deploya so quando `apps/mesas/**` muda (job `detect`), nao mais em push de infra/packages | aprovacao: commit/push -> provar (a) push de infra nao auto-deploya mesas, (b) CI quente mais rapido |
...
| BL-INFRA-DEFAULT-BRANCH | aberto | `sessoes/26-06-16_14`, pedido do mantenedor 2026-06-16 | default branch do repo GitHub + `.github/workflows/*` | **tornar `dev` o default branch do repo** (hoje = `main`); alinha o alvo de PR/visibilidade ao fluxo real (`feat->dev`). **Ao tocar (NAO por mim):** ANTES verificar que nao quebra workflows que assumem `main`/`dev` (`_deploy-module` resolve env por `refs/heads/dev`->beta senao prod; invariante `main ⊆ dev` / `validate_branch_invariant.sh`; cron `docker-cleanup`/`mesas-auto-archive` rodam so na default branch -> mudaria de main p/ dev; `deploy.yml` push/PR paths; Scorecard/CodeQL on:push default; branch protection/required checks) e corrigir o que quebrar na MESMA mudanca | mapear refs hardcoded, ajustar workflows/invariante/cron p/ dev-default, smoke beta+prod proporcional |
...
| BL-QA-SITE-IMAGES | pausado-beta-later | `specs/025-quality-lighthouse-program/lighthouse-findings.md`, `specs/025-quality-lighthouse-program/baseline-2026-06-16.md`, `sessoes/26-06-16_6_quality_perf-images-robots.md`, `sessoes/26-06-16_9_quality-beta-deploy.md`, `docs/agents/infra-map.md` | `apps/site`, Cloudinary, cards/posts/footer | `Card.astro` reserva dimensoes e prepara `srcset/sizes`; WP atual nao reduz bytes por query e derivados testados deram 404; Lighthouse beta mobile ja melhorou 85 -> 95 e LCP 3471ms -> 2606ms. Secret `CLOUDINARY_URL` existe no GitHub Environment `beta`, mas migração real/upload em massa foi pausada pelo mantenedor ate `beta.artificiorpg.com` estar pronto/on | retomar depois com aprovacao nominal para materializar env/flag (`SITE_MIGRATE_MEDIA=true`) e executar migração beta; nunca expor secret nem escrever WP |
