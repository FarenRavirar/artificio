...
|---|---|---|---|---|---|
| BL-REALIP-023 | local | `specs/023-infra-real-ip-contract/` | ingress, `accounts`, `site`, `glossario`, `mesas` | commit/push/PR/deploy + `nginx -t` e smoke runtime beta/prod | pedir autorizacao nominal para publicar; depois validar configs renderizadas |
...
| D-FEEDBACK1 | parcial | `specs/021-feedback-site-glossario/`, `project-state.md` | site + glossario | glossario foi para prod; site fica vivo em beta enquanto Gate C mantem WP na raiz | validar admin/beta quando retomar site; nao acionar prod raiz antes do Gate C |
| D-CONT1 | validacao | `sessoes/26-06-12_2_debitos_ux-marca.md` | changelogs mesas/glossario | confirmar se changelogs recentes chegaram a prod; se nao, publicar com proximo release | checar diff/estado atual antes de agir |
| D-MESAS-UI1 | local | `sessoes/26-06-12_2_debitos_ux-marca.md` | `apps/mesas` frontend | contraste do botao/badge Arquivar corrigido localmente; falta publicar se ainda nao entrou | verificar diff atual; se pendente, incluir em PR de mesas |
...
| D-NGINX1/D-NGINX2 | absorvido | spec 023 / D069 | Virou contrato Real IP geral; acionar BL-REALIP-023. |
