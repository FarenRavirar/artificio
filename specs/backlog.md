...
| BL-DEP-MESAS-DISPATCH-ENV | local | `sessoes/26-06-16_9_quality-beta-deploy.md`, run `27629745457` | `.github/workflows/deploy-mesas.yml` | expressão local corrigida para `github.ref == refs/heads/dev ? beta : prod`; falta commit/push e validação por Actions | após aprovação nominal: commitar/pushar e rodar dispatch `deploy-mesas.yml --ref dev -f mode=deploy`, verificando job `Deploy mesas beta` |
| BL-MESAS-BETA-MIGRATION-DRIFT | validacao | `sessoes/26-06-16_9_quality-beta-deploy.md`, run `27629743341` | mesas beta DB + migrations | falha original dizia que `migration_05_aggregator_sources_and_queue.sql` estava ausente; inspeção posterior mostrou arquivo presente em `/opt/artificio-beta` no commit `4de6f6a`, e reexecução do script de migrations retornou `schema em conformidade` | provar por novo deploy beta após correção do workflow; se voltar a falhar, investigar corrida/reset/estado de `schema_migrations` |
