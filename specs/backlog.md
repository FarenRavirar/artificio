...
| BL-INFRA-CACHE-CAP-F10 | local | `specs/026-infra-workflows-audit/` F10, `sessoes/26-06-16_10`, `_deploy-module.yml`, `docker-cleanup.yml` | build cache Docker VM | IMPLEMENTADO local (revisado): inspecao VM provou que imagens nao acumulam (1 tag/repo, dangling=0); vilao = build cache ~20GB <7d, e build usa `--no-cache`. Fix: `docker builder prune -f` total pos-deploy no `_deploy-module` + no semanal. Cap de imagem descartado (no-op), scripts removidos | bloqueado por aprovacao: commit/push + 1 deploy provando `system df` cair (build cache ~0) |
...
| BL-DEP-MESAS-DISPATCH-ENV | fechado | `sessoes/26-06-16_9_quality-beta-deploy.md`, runs `27629745457`, `27630434690`, commit `485b363` | `.github/workflows/deploy-mesas.yml` | workflow corrigido: dispatch em `refs/heads/dev` virou `mesas / Deploy mesas beta`; run `27630434690` verde | monitorar proximo dispatch em main/prod; nao reabrir sem nova evidencia |
