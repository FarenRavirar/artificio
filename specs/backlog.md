...
| BL-DEP-PATHFILTERS | local | `docs/agents/deploy-flow.md`, `specs/026` F1 | GitHub Actions | IMPLEMENTADO local: manifests raiz (`package.json`/`pnpm-lock.yaml`/`pnpm-workspace.yaml`/`turbo.json`) adicionados aos `paths:` de mesas/glossario/site | aprovacao: commit/push + provar que bump de dep dispara CI de modulo |
