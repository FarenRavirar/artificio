...
- [ ] TZ.3 — Smoke visual do mantenedor em beta. Fluxo: (1) commit local na branch `feat/054-gestao-ia-sidebar` → (2) push → (3) abrir PR para `dev` (branch protection exige check `lint+build+test` verde) → (4) CI verde → (5) **merge PR em `dev`** (⚠️ `_deploy-module.yml` hard-coded `git reset --hard origin/dev` — deploy beta só enxerga código mergeado) → (6) deploy beta mesas (`gh workflow run deploy.yml --ref dev -f module=mesas -f mode=deploy -f env=beta`) → (7) smoke autenticado em `mesasbeta.artificiorpg.com/gestao` → (8) se smoke falhar, reverter/fixar; se OK, promote `dev→main` ff → (9) deploy prod mesas (gated por aprovação nominal). **Não pular etapas.**
