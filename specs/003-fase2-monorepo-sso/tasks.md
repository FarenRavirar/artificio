...
Tunnel correto `<TUNNEL_UUID>` → Public Hostname `accounts.artificiorpg.com` → `http://accounts-api:3000`. DNS resolve via Cloudflare. `502 Bad Gateway` atual é esperado até `accounts-api:3000` subir.
