...
56d. **Escrita no catálogo central atravessa fronteira de serviço, e isso muda dois requisitos de robustez — 2ª rodada de review da PR #218 (Codex, 2026-07-27).** O catálogo vive no banco do **Site**; a transação do Downloads não o alcança. Dois defeitos reais decorriam disso:

   - **Acréscimo de alias não era atômico.** `addCatalogMaterialTypeAlias` lia a lista, concatenava e reenviava tudo (`aliases: [...atuais, novo]`). Duas aprovações simultâneas para o **mesmo tipo** liam a mesma lista e a última gravação apagava o alias da primeira — as duas sugestões ficavam `approved`, mas só um alias sobrevivia, e o `raw_value` perdido voltaria à fila na ingestão seguinte. Decisão do mantenedor: corrigir na origem, em `apps/site` (ampliação de escopo autorizada), com `add_aliases` fazendo a concatenação **dentro do UPDATE** (`jsonb_agg(DISTINCT ...)` sobre `aliases || $n`), sem janela entre ler e escrever, e deduplicando em SQL. O `aliases` de substituição continua existindo para edição admin deliberada.
   - **Tipo órfão em falha parcial.** Se o POST ao catálogo concluísse e a escrita local falhasse depois, a sugestão voltava a `pending` mas o tipo permanecia criado lá; o retry colidia no `slug` UNIQUE e devolvia 500 **para sempre**, travando a fila. Compensação por `archiveCatalogMaterialType` (status `merged`, não `rejected` — o tipo não foi recusado por um humano, apenas nunca chegou a existir de fato), no mesmo padrão que `resolveCreateSystem` já usava com `archiveCatalogNode`.

56e. **O mesmo defeito de atomicidade no fluxo de SISTEMA, corrigido por decisão do mantenedor (2026-07-27) — DEB-088-04.** Registrado inicialmente como débito herdado (está em produção desde a spec 086, não é regressão desta spec), foi reaberto e fechado no mesmo dia. `addCatalogNodeAlias` fazia o mesmo read-modify-write que o de tipo, e `replaceAliases` faz `DELETE` + `INSERT`.

   O levantamento mostrou o escopo **menor** do que o débito supunha: `addCatalogNodeAlias` tem **um único consumidor** (a triagem de sistema do Downloads). `mesas` e `glossario` escrevem no catálogo, mas sempre com create/update de lista completa — nunca append. Logo o campo novo é aditivo e não altera o comportamento de nenhum dos dois, o que a suíte confirmou (mesas 607/607, glossário 22/22 sem tocar em uma linha deles).

   Solução: `addAliases` em `apps/site/db/repo/catalog.ts`, com `INSERT ... ON CONFLICT (node_id, lower(alias), COALESCE(locale,'')) DO NOTHING` — **sem migration**, porque o índice único existe desde a `006_catalog_foundation`. Cada alias é um INSERT independente e idempotente: sem `DELETE`, não há o que uma transação concorrente apague. `replaceAliases` permanece intacto para edição admin deliberada — transformá-lo em acréscimo tornaria impossível **remover** um alias errado.

   Provado contra Postgres real (PGlite), não contra mock de query: a garantia é do banco (índice + `ON CONFLICT`), e um mock provaria apenas que a string SQL foi montada. O teste decisivo é o de dois acréscimos independentes preservando ambos os aliases — exatamente o caso que o read-modify-write perdia.

56f. **Normalização de payload que não chega ao uso — achado de review da PR #218 (CodeRabbit, 2026-07-27).** A rota `/resolve` validava o corpo com Zod (`.trim()`, `.max(200)`) e em seguida entregava **`req.body` cru** aos resolvers, que liam o valor original por `readTrimmed(ctx.body.name)`. Corrigido passando `parsed.data` — `looseObject` preserva as chaves extras que cada resolver consome, então o que muda é apenas que os campos declarados chegam normalizados.

   > **Precisão do achado, corrigida na 2ª passada do CodeRabbit:** a redação inicial deste requisito (e os comentários no código) afirmava que payload **acima do limite** chegava ao catálogo central. Isso é falso — `safeParse` roda antes de qualquer resolver e rejeita com 400. O defeito real é mais estreito: o payload **válido** seguia sem transformação, ou seja, nome com espaços nas pontas ia para `createCatalogMaterialType` exatamente como veio. O `.max()` sempre funcionou; o que não alcançava o uso era o `.trim()`. Registrado porque exagerar a severidade de um achado corrigido é tão ruim quanto omiti-lo: leva o próximo leitor a supor um buraco de segurança que nunca existiu.
   >
   > **Não existe teste que distinga os dois caminhos hoje, e isso foi medido.** O CodeRabbit pediu um seam que falhasse caso a rota voltasse a passar `req.body`. Revertendo a linha e rodando a suíte, **23/23 continuavam passando** — porque todo campo string de `resolveBodySchema` tem `.trim()` e `readTrimmed` dentro de cada resolver apara de novo, fazendo os dois caminhos convergirem para o mesmo valor. `node_type` (enum, sem trim) também não serve: o `safeParse` rejeita antes, então o 400 aparece nos dois casos. Consequência honesta: `parsed.data` aqui é **defesa em profundidade**, não correção de bug observável — a normalização já ocorria por acidente do `readTrimmed`. O ganho é de contrato (resolver novo não precisa lembrar de aparar). Um seam real exigiria remover `readTrimmed` dos resolvers e confiar só no schema — refactor amplo em rota de produção, além do que o achado pedia.

   **O mesmo defeito existia em `systemSuggestionsAdmin.ts:307`**, de onde o padrão foi copiado — em produção desde a spec 086, não é regressão desta spec. Corrigido junto por decisão do mantenedor (2026-07-27), com a razão explícita de que deixar os dois divergentes garantiria que a próxima cópia levasse o errado de novo. Isso encerra a origem do defeito, não só a instância nova.

56g. **`aliases` e `add_aliases` são mutuamente exclusivos — achado de review da PR #218 (CodeRabbit, 2026-07-27).** O review supôs perda silenciosa ("a última gravação vence"). Verificado contra Postgres real: o efeito é **pior** — os dois campos no mesmo patch produzem duas atribuições à mesma coluna e o Postgres recusa com `multiple assignments to same column "aliases"`, o que chegaria ao cliente como **500**. Um comentário anterior neste código afirmava que os dois "podem vir juntos"; era suposição não testada e foi removido.

   Rejeitado com `aliases_conflict` → **400**, em vez de eleger um dos dois: substituir e acrescentar na mesma chamada não tem leitura óbvia, e adivinhar apagaria vocabulário sem o chamador pedir. `add_aliases: []` **não** conflita — lista vazia é ausência de intenção, não intenção contraditória, e bloqueá-la quebraria um PATCH legítimo que só troca a lista.

   A mesma trava foi aplicada a `updateCatalogNode` (nodes). Lá o Postgres **não** acusaria nada, porque `replaceAliases` e `addAliases` são statements separados — o que é pior: passaria em silêncio com uma semântica que ninguém especificou.
