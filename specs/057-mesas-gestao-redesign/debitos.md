...
## Investigação em aberto — pausada a pedido do mantenedor

- **DEB-057-18** — **PAUSADO (2026-07-01).** Validação de token de usuário/session (`POST /chat-exporter/validate-token`, botão auto no `onBlur`) devolve 401 pra token confirmado válido pelo mantenedor (usado normalmente no `DiscordChatExporter.Cli`). Duas hipóteses testadas e descartadas: (1) falta de `User-Agent` — comparação com fonte upstream (`DiscordClient.cs`/`Utils/Http.cs`) mostra que o CLI também manda só `Authorization`, sem headers extra; (2) token corrompido — análise errada, decodificação de snowflake não se aplica a token de usuário/self atual. Hipótese líder não testada: Discord bloqueia/penaliza IP de datacenter/cloud pra token de usuário/self (a chamada de teste rodou de ambiente sandbox, não da VM real nem do PC do mantenedor). Se confirmado, pode afetar a exportação real (CLI roda na mesma VM), não só a validação. Evidência completa em `chatexporter-solucao.md` §1 "Achado (2026-07-01)". Próximo passo (quando retomado): testar `GET /users/@me` a partir da VM beta real (SSH read-only) pra confirmar/descartar bloqueio por IP. **Não retomar sem pedido explícito do mantenedor** — próxima etapa combinada é ele validar exportação manual + importação de JSON pra testar o resto do fluxo sem depender do modo usuário/session.
