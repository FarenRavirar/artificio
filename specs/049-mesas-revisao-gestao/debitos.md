...
| D04 | ~~Nenhuma task de frontend executada (TE10-TE17)~~ | ~~Fase E parcial~~ | **RESOLVIDO:** 2 hooks (useDiscordSync, useJsonImport) + 5 componentes (ImportResultGrid, JsonPreviewCard, FileDropzone, MessagesToolbar, GestaoStateWrapper, StatCard). DiscordSyncPanel 543→308, DiscordJsonImportPanel 356→115. Build 17/17 ✅, tests 228/228 ✅. |
...
| D11 | ~~Routes produzidas (drafts.ts, settings.ts) têm `isAdmin` duplicado~~ | ~~Herdado do padrão TE1-TE4~~ | **RESOLVIDO:** 6 route files migrados para `requireAdmin`. 3 test mocks atualizados. `isAdmin` removido de `inbox/utils.ts`. 42+ linhas eliminadas. Build 17/17 ✅, tests 228/228 ✅. |
