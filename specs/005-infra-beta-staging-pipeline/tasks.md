...
- [ ] T8 — Clone beta na VM (realm Opção 1, D042) · feito quando: `/opt/artificio-beta` é clone `dev` deploy key read-only; `apps/mesas/.env.beta` presente **sem OAuth**; `apps/accounts/.env.beta` presente como anchor com **só `JWT_SECRET` = valor do prod** (beta reusa realm prod); segredos fora do git. *(write VM — aprovação ✅ liberada após D042)*
