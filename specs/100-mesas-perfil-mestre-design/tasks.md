...
- [x] T5.1 — `rtk pnpm run test` → **43/43 tarefas**, 0 falhas. Rodado três vezes: entrega original (`a3738f3`), commit de review (`1262a87`) e a terceira rodada de achados, sempre com o mesmo resultado. Duas falhas transitórias apareceram e foram diagnosticadas: na primeira, `ImageFieldsAria.test.tsx` (mock literal apagava um export que eu acabara de criar — **quebra minha**, corrigida com `importOriginal`); na terceira, `@artificio/image-editor` com "no tests", que era o `dist` de `@artificio/media` sendo reescrito enquanto o teste importava — some ao rerodar isolado (15/15).
- [x] T5.2 — `rtk pnpm run lint` → **26/26**, **0 erros**, nas três rodadas. 1 warning `react-hooks/exhaustive-deps` em `useBannerScrim.ts:251`, arquivo não tocado nesta spec e warning anterior a ela.
- [x] T5.3 — `rtk pnpm run build` → **26/26**, 0 falhas, nas três rodadas.
...
- [x] F6.4d — **FEITO.** `ImageUploader.test.tsx`, **7 casos** — arquivo novo, o componente não tinha teste algum. Os 4 últimos vieram das rodadas de review: os dois caminhos de troca com o campo aberto, o Cloudinary de terceiro, e a asserção de foco. Todos verificados por reversão.
