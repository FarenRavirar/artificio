...
- [x] T5.1 — `rtk pnpm run test` → **43/43 tarefas**, 0 falhas. Na primeira execução falhou 3 (`ImageFieldsAria.test.tsx`): o mock literal de `useImageUrlImport` apagava `isCloudinaryUrl`, que passei a exportar em F6.4c — **quebra introduzida por mim nesta sessão**, corrigida com `importOriginal`.
- [x] T5.2 — `rtk pnpm run lint` → **26/26**, **0 erros**. 1 warning `react-hooks/exhaustive-deps` em `useBannerScrim.ts:251`, arquivo não tocado nesta sessão e warning anterior a ela.
- [x] T5.3 — `rtk pnpm run build` → **26/26**, 0 falhas.
...
- [x] F6.3d — **FEITO, e num lugar diferente do que esta task supunha.** A guarda **não** entrou em `createSystemNode`: `mesas` e `downloads` têm cada um o seu, e os dois delegam ao mesmo endpoint do site — pô-la num app deixaria o outro aberto, e o próximo app nasceria aberto também. Entrou em `apps/site/db/repo/catalog.ts` (`assertNoEquivalentSibling`, chamada por `createNode`), que é a fábrica real. Compara nome e aliases normalizados (sem acento, caixa, pontuação) e **remove o nome do pai do começo**, que é o que faz `2024` e `Dungeons & Dragons 2024` colidirem sob o mesmo pai. `409 duplicate_sibling_node` traduzido nos dois apps — sem isso viraria 500 no `downloads` e "já existe com este caminho" (mensagem errada) no `mesas`. **Só CRIAÇÃO:** `updateNode` fica de fora de propósito, senão F6.3e não poderia renomear para o canônico enquanto o duplicado existe. Medido: 5 testes novos em `catalog.test.ts` contra Postgres real, 18/18; **revertida a chamada, 3 falham** (o "5e" exato já era pego pela UNIQUE de `path_slug`; os outros dois são exatamente o furo). 
...
- [x] F6.3f — **FEITO.** `gm.publicSystems.test.ts`, 4 casos: sistemas gravados saem com a cadeia inteira; mestre sem sistema devolve `[]`; id fantasma é descartado em vez de virar nome nulo; catálogo fora do ar não derruba o perfil. 
...
- [x] F6.4c — **FEITO.** Com imagem já hospedada no Artifício, o campo de link some e ficam a prévia (que já existia) e "Trocar por um link de imagem". Critério: `isCloudinaryUrl`, exportado de `useImageUrlImport` — o MESMO predicado que decide importar, para não haver duas noções de "hospedada". **Link externo mantido por "Manter link direto" continua visível:** aquela URL o mestre digitou, e escondê-la tiraria a única forma de conferí-la. **`AvatarField` não precisou de mudança** (medido): o campo dele já vive num `<details>` fechado. O defeito era só do `ImageUploader` — o banner, que é o campo que o mantenedor viu. 
- [x] F6.4d — **FEITO.** `ImageUploader.test.tsx`, 4 casos — arquivo novo, o componente não tinha teste algum. 
...
- [x] F7.1c — **FEITO.** Lápis (`lucide-react`) permanente à direita do valor em `ProfileFieldRow`, `opacity: 0.65` em repouso e cheio no hover/foco — presente nos dois estados, nos dois temas, sem depender de gesto nem de cor. `aria-label={`Editar ${label}`}`, porque o nome acessível anterior era só o rótulo e lia como texto. Sem tamanho, peso ou raio novos.
...
- [x] F7.5e — **FEITO**, junto com a mudança de posição de D31.
