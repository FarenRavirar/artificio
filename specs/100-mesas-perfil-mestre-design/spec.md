...
- **D33 — A projeção de beta PODE criar nó novo; o que ela não pode é criar duplicata (2026-09-08).** Levantada ao implementar F6.3d, quando perguntei se `createLocalNode` deveria simplesmente recusar criação e mandar tudo ao Central. **Resposta do mantenedor: "era para a projeção de beta poder criar, para fazer testes, senão para que serve ter um beta?"** — beta que não exercita o fluxo de criação não testa justamente o que se quer validar antes de produção.

  **O que isto NÃO afrouxa.** A fonte canônica continua sendo o **Central, no Site Prod** (D114, 2026-07-15, firme): *todos* os ambientes de produção consomem o compartilhado — medido em 2026-09-08: `mesas` prod usa `APP_ENV=production` → `central`, e `downloads` e `glossario` não têm bifurcação nenhuma, usam o Central sempre. A projeção local existe **só em beta**, hidratada do Central por upsert com UUID preservado.

  **Consequência para a guarda:** o defeito nunca foi *criar* em beta — foi criar `5e` duas vezes. Por isso F6.3d é guarda de unicidade nos dois lados (Central e projeção), e não proibição de criação local. Também explica sem mistério por que beta divergiu e produção não: beta acumula nós de teste que nunca foram ao Central, e é para isso que serve.
