...
- 2026-06-05 — CDX-309C: `_enforce-migration-dir.yml` trocado para glob bash nativo sem `sed` + self-test permitido/bloqueado. Migration lock refeito: `flock` interno segura execução inteira; cada migration usa `pg_advisory_xact_lock` dentro da transaction. Adicionado `scripts/deploy/test_migration_lock.sh`; Git Bash local não tem `flock`, então prova concorrente roda no Actions Ubuntu via `_lint-shell.yml`.
- 2026-06-05 — CDX-309D: PR #1 verde/clean mergeado por squash em `main` (`3cf08d0`). Parte C executada com aprovação: backup off-VM antes, pré-requisitos VM OK (`git`, Docker/Compose, `flock`, `curl`, `jq`), deploy key read-only criada, `/opt/artificio` convertido para clone `main`, `.env` de `accounts`/`mesas` preservados, `git status` limpo na VM. Sem deploy, sem build, sem migration, sem `workflow_dispatch`.
- 2026-06-05 — Validação pós-conversão CDX-309D: containers seguiram `Up`; `mesas-api`, `mesas-app`, `accounts-db`, `mesas-db` healthy; `accounts-api` sem healthcheck Docker mas `running` e smoke `/health=200`; `mesas /=200`; `mesas /api/v1/me/options` sem cookie = `401`. Criado shim operacional local na VM `apps/accounts/docker-compose.prod.yml -> docker-compose.yml` (gitignored) para compatibilizar checklist; reconciliar em CDX-310 como arquivo versionado correto.
- 2026-06-05 — CDX-309E: 1º deploy real via pipeline canônico `deploy-mesas.yml workflow_dispatch mode=deploy`. Primeira tentativa falhou antes de snapshot por comparação crua de `JWT_SECRET` com CRLF; corrigido em `b5c172d`. Segunda tentativa mostrou sucesso falso porque heredoc via SSH teve stdin consumido pelo `docker compose`; corrigido em `986516b` enviando script remoto como arquivo temporário. Terceira tentativa `26994910262` passou de ponta a ponta: `jwt_secret_shared=true`, snapshot criado, migrations `125/126` aplicadas, compose rebuild/recreate, `mesas-api`/`mesas-app` healthy, smoke `home=200`, `private_no_cookie=401`, `auth_redirect=302`.
- 2026-06-05 — Validação pós-CDX-309E: VM em `986516b`, `git status` limpo; containers `mesas-db`, `mesas-api`, `mesas-app` healthy e `mesas-cron` running/no-health; DB `tables=49`, `users=15`, `schema_migrations=55`; snapshot temporário removido após sucesso; sem rollback.
