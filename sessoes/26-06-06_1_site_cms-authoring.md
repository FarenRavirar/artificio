...
- **COMMITADO + PUSHADO + DEPLOYADO no beta (autorizado por ação 2026-06-08).** Commit `11f1736` `feat(site): add editorial status operations` (`22fe461..11f1736 dev`, 15 arq — stage seletivo; arquivos pré-modificados de Gate D/docker/handoff deixados ao mantenedor). CI verde (pr-checks/deploy-site CI/deploy-accounts/promote-dev-to-main). `deploy-site mode=deploy` run `27139842102` verde (`Deploy site beta=success`). Smoke: healthz=200(posts:125), público/admin/sitemap/rss/robots=200, `/admin/status`+`/api/admin/v1/posts`=401, WP raiz=200. `.pgdata` local recriado via `pnpm sync` (125/125) após corrupção por kill durante E2E; seeds revertidos. Arquivos alterados: `apps/site/db/repo/{posts,pages}.ts`, `apps/site/server/admin-api.ts`, `apps/site/src/pages/rss.xml.ts`, `apps/site/scripts/t17-validate.ts` (novo), `apps/site-admin/src/{api.ts,styles.css,pages/{PostsList,PagesList,PostEditor,PageEditor}.tsx}`.
...
## FECHAMENTO Fase 2 — mídia (2026-06-08)
- **T18 (backend mídia) + T19 (UI) ✅ deployados no beta.** Migration `004_media_library.sql`; API `/api/admin/v1/media` (GET busca/filtro/paginação, POST multipart com validação de MIME real `file-type` + rejeita SVG/415, PUT, DELETE); `media-store` (Cloudinary `upload_stream` ou local `/uploads`); UI Mídia (grid/upload/editar/apagar) + `MediaPicker` (featured/OG/inserir no editor) + `BlockEditor.insertImage`. Deps `multer`+`file-type`. Validação: `t18-validate` 10/10 + smoke HTTP.
- **Cloudinary no beta:** 3 vars (`CLOUDINARY_CLOUD_NAME/API_KEY/API_SECRET`) reusadas do `.env.beta` do mesas (mesma conta). `secrets.7z` do backup deu erro de senha/corrupção — não usado.
- **Incidente 502 + fix:** ativar Cloudinary fez o import-on-start subir ~485 mídias WP no boot → healthcheck estourou → beta caiu. Fix `e5ee84e`: **migração em massa WP→Cloudinary virou opt-in `SITE_MIGRATE_MEDIA=true`** (default off = boot rápido); uploads novos do admin seguem no Cloudinary. Redeploy verde, beta recuperado.
- Commits: `292dd06` (mídia) · `206c780` (compose 3 vars) · `e5ee84e` (boot-fix) · docs. `main=dev`.
- **Próximo na fila:** **T20 — CRUD de taxonomias completo** (fecha Fase 2), depois Fase 3 (dashboard/curadoria/nav/redirects) ou CDX-310. Handoff em `docs/agents/claude-next-prompt.md`.
- **Pendência de validação (mantenedor):** E2E autenticado no beta — upload no /admin → confirmar URL Cloudinary; inserir imagem/featured/OG no editor; publicar.

## FECHAMENTO (2026-06-08)
- **T16 ✅** — E2E autenticado validado pelo mantenedor no beta (login Google real → `/admin` → criar/editar → editor legível + painel SEO Yoast → publicar → rebuild/HTML público). FECHADO.
- **T17 ✅** — operações editoriais básicas + publicação honesta + refino de UX (editor BlockNote light, painel SEO/OG Yoast-like) no ar no beta; Codex aprovou.
- **Fix de persistência de sessão SSO** (refresh-retry no cliente, `packages/auth`) entregue nesta sessão — live em beta + mesas prod (`356b650`). `main=dev`.
- **Próximo na fila:** **Fase 2 — mídia (T18 schema/API Cloudinary, T19 UI/inserção no editor)**, fechando "criar post completo sem sair do admin". Alternativa paralela: **CDX-310** (retrofit deploy accounts). Detalhe e ordem em `tasks.md` Fase 2 + `docs/agents/claude-next-prompt.md`.

## Próximos passos (retomar)
1. ~~**T16 / E2E autenticado**~~ ✅ FECHADO (mantenedor validou no beta).
