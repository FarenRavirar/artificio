...
- 2026-06-05 — PR #6 mergeado em `dev` (`cad3ba1`) e T11 estrito validado no run `27030832884`: actionlint/ShellCheck/CI/deploy beta OK; `branch_invariant_ok=true`, `jwt_secret_shared=true`, snapshot `/tmp/artificio-mesas-beta-predeploy-27030832884.dump`, migrations `schema em conformidade`, `mesas-beta-api/app` healthy, smoke `home=200`, `private_no_cookie=401`, `auth_redirect=302`. Smoke externo: `mesasbeta` `200/401/302 location ...return=https%3A%2F%2Fmesasbeta.artificiorpg.com%2F`; prod `200/401/302 location ...return=https%3A%2F%2Fmesas.artificiorpg.com%2F`; VM: beta/prod containers healthy, `/opt/artificio-beta=cad3ba1`, `/opt/artificio=c5ff42d`.
- 2026-06-05 — T12 hydrate iniciado: baseline read-only `beta NODE_ENV=development`, `prod NODE_ENV=production`, beta `users=14/tables=14/schema_migrations=55`, prod `users=15/tables=14/schema_migrations=55`. Prod gate via token efêmero interno retornou `403 ABORT` antes de tocar prod DB. Beta dry-run falhou `500`; logs indicam `duplicate key` em `table_metrics_table_id_key`. Causa: hydrate usa `ON CONFLICT id` para `table_metrics`, mas schema tem `UNIQUE(table_id)`; `gm_profile_metrics` tem padrão análogo `UNIQUE(gm_profile_id)`. Hotfix local em branch `feat/005-hydrate-metrics-conflict`: conflitos por chave semântica.

## Bloqueios / aprovações pendentes
- T12/E2E depende de aprovação para hydrate beta (write DB beta) + login browser real.
