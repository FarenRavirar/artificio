...
- 2026-06-12 — PR #20 (`fix(accounts): alinhar login à marca`) mergeado em `dev` com autorização explícita do mantenedor. Merge commit GitHub: `8ce13125f1dd543e7df90ebcf8731973a5ec9fab`. Deploy autorizado e executado via `deploy-accounts.yml --ref dev -f mode=deploy`, run `27392546006`, status success. Smoke público pós-deploy: `/health` 200, `/login` 200, `/api/auth/me` sem cookie 401. Browser live em `https://accounts.artificiorpg.com/login?return=https%3A%2F%2Fbeta.artificiorpg.com%2F`: título `Entrar · Artifício RPG`, CTA `Entrar com Google`, kicker presente, sem console errors.
- 2026-06-12 — Review automatizado apontou contraste insuficiente no hover do CTA (`#fff` sobre `#e0712f`, ~3.2:1). Fix local aplicado: hover mantém `var(--accounts-on-brand)`/navy sobre `--accounts-brand-deep`. Contraste calculado `#020740` sobre `#e0712f` = **5.94:1**. Validação local pós-fix: `pnpm --filter @artificio/accounts build` OK; `pnpm --filter @artificio/accounts test` OK (8/8).

## Pendência
- Mantenedor testar visual e fluxo real Google no browser dele.
- Fix de contraste está local; publicar exige novo commit/PR/merge/deploy com aprovação explícita.
