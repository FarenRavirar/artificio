    ...
    <div className="max-w-6xl mx-auto px-4 py-10">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Timeline consolidada de sugestões, comentários, votos, edições e moderação.</p>
        </div>
        <button
      ...
      <section className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
        ...
        {loading ? (
          <div className="p-8 text-center text-gray-500">Carregando atividade administrativa...</div>
        ) : items.length === 0 ? (
          <div className="p-8 text-center text-gray-500">Nenhum evento encontrado para os filtros aplicados.</div>
        ) : (
          <div className="divide-y divide-gray-100">
            {items.map((item) => (
              <article key={item.event_id} className="p-4 hover:bg-gray-50 transition-colors">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
                  <p className="text-sm font-semibold text-azul-escuro">{item.summary || 'Evento administrativo'}</p>
                ...
                <div className="mt-2 flex items-center gap-2">
                  {item.entity_type === 'user' ? (
                    <Link to="/admin/users" className="text-xs font-bold text-azul-medio hover:underline">
                      Abrir Gestão de Membros
                    </Link>
                  ) : (
                    <Link to="/admin/review" className="text-xs font-bold text-azul-medio hover:underline">
