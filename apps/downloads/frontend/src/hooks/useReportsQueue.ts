...
export function useReportDecision() {
  ...
  return useMutation({
    mutationFn: async ({ id, case_state, resolution_note, priority }: { id: string; case_state: 'in_review' | 'resolved' | 'dismissed'; resolution_note?: string; priority?: 'P0' | 'P1' | 'P2' | 'P3' }) => {
      const response = await apiPatch(`/api/v1/reports/${id}`, { case_state, resolution_note, priority });
      if (!response.ok) {
