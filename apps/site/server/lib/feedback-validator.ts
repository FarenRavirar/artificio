...
// Port de apps/mesas/.../devFeedbackValidator.ts. Entrada tratada como `unknown`
// (sem map/spread antes de validar). Limites espelham FEEDBACK_LIMITS de
// @artificio/ui/feedback — mantidos locais (fronteira de segurança server-side).
...
export interface ConsoleErrorEntry { level: string; message: string; ts: string | null; }
export interface NetworkErrorEntry { url: string; method: string; status: number; ts: string | null; }
...
function readTruncatedOrNull(value: unknown, max: number): string | null {
  ...
  return s.length === 0 ? null : trunc(s, max);
}
...
function normalizeConsoleErrors(value: unknown): ConsoleErrorEntry[] {
  ...
  for (const item of value) {
    ...
    out.push({ level: trunc(level, LEVEL_MAX), message: trunc(message, FEEDBACK_LIMITS.message), ts: ts.length > 0 ? trunc(ts, 40) : null });
  }
  return out;
}
...
function normalizeNetworkErrors(value: unknown): NetworkErrorEntry[] {
  ...
  for (const item of value) {
    ...
    const ts = readString(row.ts).trim();
    out.push({ url: trunc(url, FEEDBACK_LIMITS.url), method: trunc(method, 10), status: Math.trunc(row.status), ts: ts.length > 0 ? trunc(ts, 40) : null });
  }
  return out;
}
...
export function parseFeedbackInput(raw: unknown): ParseResult {
  ...
  if (title.length === 0) return { ok: false, error: "Titulo e obrigatorio." };
  const description = readString(body.description).trim();
  if (description.length === 0) return { ok: false, error: "Descricao e obrigatoria." };
