...
DO $$
DECLARE
  source_id constant text := 'ac74d486-e3d3-4635-b7a0-7847f04050f5';
  target_id constant text := 'c3d31503-b4af-4663-af10-c1ef062102c3';
  source_ok boolean;
  target_ok boolean;
  next_version bigint;
BEGIN
  SELECT EXISTS (
    ...
    WHERE id = source_id AND name = '2024' AND node_type = 'edition'
      AND parent_id = '36698ed7-2d51-4edb-95ea-b1fcddd1e9c9'
  ) INTO source_ok;
  SELECT EXISTS (
    SELECT 1 FROM catalog_nodes
    WHERE id = target_id AND name = '2024' AND node_type = 'variant'
  ...
  END IF;
  IF EXISTS (SELECT 1 FROM catalog_nodes WHERE parent_id = source_id) THEN
    RAISE EXCEPTION 'dnd_2024_source_has_children';
  END IF;

  UPDATE catalog_legacy_mappings SET canonical_id = target_id, updated_at = now()
  WHERE canonical_id = source_id;
  UPDATE catalog_suggestions SET resolved_node_id = target_id WHERE resolved_node_id = source_id;
  ...
  VALUES (source_id, target_id, 'D&D 2024 reclassificado como variante de 5e')
  ON CONFLICT (source_id) DO UPDATE SET target_id = EXCLUDED.target_id, reason = EXCLUDED.reason;

  UPDATE catalog_nodes
  SET status = 'merged', merged_into_id = target_id, updated_at = now(), version = version + 1
  WHERE id = source_id;
  ...
  VALUES (source_id, 'catalog_node_reclassified', jsonb_build_object(
