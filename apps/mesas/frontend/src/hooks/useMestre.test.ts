...
describe('normalizeSellingPoints', () => {
  ...
  // Achado de review (#297): "   " tem length 3 e passava pelo filtro antigo,
  // renderizando destaque em branco. O formulario ja usava trim
  // (`isValidSellingPoint`) — as duas camadas divergiam.
  it('filtra title/description so com espacos em branco', () => {
    const entrada = [
      { icon: 'clock', title: '   ', description: 'ok' },
      { icon: 'clock', title: 'ok', description: '    ' },
      { icon: 'clock', title: ' ', description: ' ' },
    ];
    expect(normalizeSellingPoints(entrada)).toEqual([]);
  });
...
describe('normalizeMestreProfile — selling_points na fronteira', () => {
  it('normaliza {} (forma real do beta) para []', () => {
    const out = normalizeMestreProfile(
      makeMestreProfile({ selling_points: {} as unknown as SellingPoint[] }),
    );
  ...
  it('preserva os demais campos normalizados (avatar/banner/avg_rating)', () => {
    const out = normalizeMestreProfile(
      makeMestreProfile({ selling_points: {} as unknown as SellingPoint[], avg_rating: '4.50' as unknown as number }),
    );
