...
export function ImportResultGrid({ result, onNavigateToDrafts }: ImportResultGridProps) {
  const autoParse = result.auto_parse;
  ...
  return (
    <div className="rounded-lg bg-green-900/20 border border-green-600/30 p-4 space-y-3">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-2">
        ...
        <StatCard label={autoParse ? 'Rascunhos' : 'Ignoradas'} value={autoParse?.parsed ?? result.ignored} valueClassName="text-white/70 text-lg font-bold" />
      ...
      {autoParse && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
          <StatCard label="Apuradas" value={autoParse.total} />
          <StatCard label="Descartadas" value={autoParse.discarded} valueClassName="text-amber-300 text-lg font-bold" />
          <StatCard label="Inválidas" value={autoParse.ignored} valueClassName="text-white/70 text-lg font-bold" />
          <StatCard label="Erros de apuração" value={autoParse.errors} valueClassName={`text-lg font-bold ${autoParse.errors > 0 ? 'text-red-300' : 'text-white/70'}`} />
        </div>
      )}
      <div className="flex items-center justify-between">
        <p className="text-white/50 text-xs">
          {autoParse
            ? 'Os anúncios válidos foram convertidos em rascunhos revisáveis.'
            : 'As mensagens importadas estão com status "Pendente". Apure-as para gerar drafts revisáveis.'}
        </p>
        {onNavigateToDrafts && (
          <button
