    ...
    <section className="container" style={{ marginTop: '3rem' }}>
      ...
      {isAuthenticated ? (
        <div className="mb-6 rounded-xl border border-[var(--line)] bg-[var(--fill-subtle)] p-4">
          ...
          <button type="button" onClick={() => void handleSubmit({ rating, tags, comment: comment.trim() })} disabled={rating < 1 || isSubmitting || contentOverflow(comment, REVIEW_COMMENT_MAX_LENGTH) > 0} className="mt-3 rounded-lg bg-artificio-orange px-4 py-2 font-semibold text-white disabled:opacity-50">
            Enviar avaliação
          </button>
        </div>
      ) : (
