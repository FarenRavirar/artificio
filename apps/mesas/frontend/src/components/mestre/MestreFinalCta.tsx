...
export function MestreFinalCta({ totalOpenSlots, tablesCount, mappedTables }: Props) {
  ...
  // Sem campo `emoji` (T3.4): o emoji renderiza na fonte do SO e muda de
  // desenho entre plataformas, então o mesmo título não fica igual para dois
  // visitantes. Removido o campo, não só o uso — deixá-lo no tipo convidaria
  // o próximo cenário a repor o emoji sem ninguém notar.
  let ctaData: { title: string; subtitle: string; cta: string; hint: string | null };
