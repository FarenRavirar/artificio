...
/** Coerção segura p/ string — evita '[object Object]' de String() em payload externo unknown. */
...
function extractUrlsFromText(rawText: string): string[] {
  return [...new Set(
    (rawText.match(/https?:\/\/[^\s)>\]]+/gi) ?? [])
      .map((url) => stripTrailingPunct(url).toLowerCase()),
...
function buildFeatures(message: ParseLearningMessage, normalizedText: string): Record<string, unknown> {
  ...
  const rawText = toSafeString(message.content_raw);
  ...
  const embedUrls = embeds
    ...
    .map((url) => url.toLowerCase())
    .sort((a, b) => a.localeCompare(b));
  const formUrls = [...new Set([...textUrls, ...embedUrls].filter((url) => /forms\.gle|docs\.google\.com\/forms/i.test(url)))].sort((a, b) => a.localeCompare(b));
  return {
...
export function buildParseCaseContract(input: {
  ...
  // em mensagens attachment-only ou texto igual com anexos diferentes — antes,
  // dedup pescava esses casos como duplicata forte incorretamente.
  const structuralSignature = JSON.stringify({
    attachment_urls: features.attachment_urls ?? [],
    embed_urls: features.embed_urls ?? [],
    form_urls: features.form_urls ?? [],
  });
  const contract: ParseCaseContract = {
    ...
    raw_hash: sha256(`${rawText}\u0000${structuralSignature}`),
    normalized_hash: sha256(`${normalizedText}\u0000${structuralSignature}`),
  ...
  try {
    ...
    if (insertedId) {
      ...
      void recordParseLayerShadowDecisions({
        ...
        deterministicResult: contract.deterministic_result_json,
        finalResult: contract.final_result_json,
        finalAction: contract.final_action,
      });
    }
    return insertedId;
  } catch (error: unknown) {
