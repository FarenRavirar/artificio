...
- **Prevenção:** reforça a de E016 — a auditoria estática (grep de imports `@artificio/*` do código-fonte × lista de `COPY` do Dockerfile) precisa rodar **toda vez que uma dependency workspace nova é adicionada** a um `package.json` de app com Dockerfile de produção, não só quando o Dockerfile em si é editado. Débito aberto (mesmo de E016, ainda não fechado): gate de CI que faça esse cruzamento automaticamente no PR, antes do deploy real expor o gap.
- **Achado adicional (débito registrado, não investigado agora — decisão do mantenedor):** `trap rollback ERR` do script de deploy não executou a chamada real quando o erro ocorreu em runtime pós-`docker compose up` bem-sucedido (o `up` "funcionou", o crash veio depois, fora da cadeia de comandos que o `trap` cobre). Beta é ambiente de teste, sem urgência; revisar o comportamento do `trap rollback` pra falha de runtime tardia (pós-`up`) fica pendente pra sessão futura — registrar em `specs/backlog.md`.
