...
16. **Spec 049** (revisão da aba /gestao do mesas) — **FECHADA (2026-06-24).** Fases A-D (investigação+auditorias+duplicação+proposta) + Fase E (refatoração completa: TE1-TE22, 4 commits locais, 76 arquivos modificados, backend adminDiscordSync 1278→708 linhas, DiscordSyncPanel 543→308, DiscordJsonImportPanel 356→115, +2 hooks +5 componentes, +40 testes backend, +146 testes frontend) + Fase F parcial (TF1-TF6 verdes, TF7/TF11 skip). Reviews REV-001 a REV-038 implementados (incluindo D15 CC56 + D16-D18 duplicação). Débitos resolvidos: D01-D05, D08, D11-D18. Débitos abertos: D06 (8 P1), D07 (nome de teste), D09 (adminDiscordSync split), D10 (adminImportInbox split). Branch `feat/049-revisao-gestao` com 4 commits locais, código **não mergeado em `dev`**.
...
- 2026-06-24 — **Spec 049 encerrada (REV-036-038 + D15).** D15 resolvido via REV-032 (calcMissingFields + createImportMessage extraídos). REV-036 (`parseDiscordMessage` em utils.ts, 20 linhas eliminadas), REV-037 (`validateDraftStatusTransition` em utils.ts, formato erro 422 padronizado), REV-038 (`patchDraftSchema` unificado de inbox/utils.ts). 4 commits locais no branch `feat/049-revisao-gestao`. Build 17/17, lint 15/15, testes backend 223/223, frontend 163/163. Débitos resolvidos: D01-D05, D08, D11-D18. Débitos abertos: D06 (8 P1), D07, D09, D10. Código **não mergeado em `dev`**.
