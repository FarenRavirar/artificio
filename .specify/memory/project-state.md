...
16. **Spec 049** (revisão da aba /gestao do mesas) — **FECHADA (2026-06-23).** Fases A-D (investigação+auditorias+duplicação+proposta) + Fase E (refatoração completa: TE1-TE22, 3 commits locais, 13 arquivos modificados, backend adminDiscordSync 1278→708 linhas, DiscordSyncPanel 543→308, DiscordJsonImportPanel 356→115, +2 hooks +5 componentes, +40 testes backend, +146 testes frontend) + Fase F parcial (TF1-TF6 verdes, TF7/TF11 skip). Reviews REV-001 a REV-035 implementados. Branch `feat/049-revisao-gestao` com 3 commits locais, código **não mergeado em `dev`**. Pendente: D15 (CC 56), commit/push/PR.
...
- 2026-06-23 — **Spec 049 Fase E (refatoração) e Fase F (verificação) executadas.** 3 commits locais (`774c462`, `cfe6a96`, `a4d2fb5`) no branch `feat/049-revisao-gestao`. 13 arquivos modificados. TE1-TE22: backend extraído em 6 módulos (sync, import, preview, settings, drafts, messageParse, utils), frontend extraído em 2 hooks + 5 componentes (ImportResultGrid, JsonPreviewCard, FileDropzone, MessagesToolbar, GestaoStateWrapper, StatCard). Testes: backend 28 files 223/223, frontend 15 files 163/163. Build 17/17, Lint 15/15. Diagnostics zero. TE23 skip (coberto por testes). Fase F: TF1-TF6 verdes, TF7/TF11 skip. Pendência: D15 (CC 56 adminImportInbox), commit/push/PR para `dev`. Spec **fechada**, código não mergeado.
