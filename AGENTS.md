...
- **Escrever o resultado, não o caminho até ele.** Vale igual em doc e em resposta de chat: diz o que **vale agora** e a medição que sustenta. Fora, sempre: hipótese descartada, premissa que se provou falsa, o que a versão anterior dizia, por que uma opção foi abandonada, o que o agente considerou antes de decidir, recapitulação do que acabou de ser feito. Teste único: se não muda o que o mantenedor faz a seguir, não entra. Ex.: "emitimos só `Product`+`Offer`" — não "`Event` foi cancelado porque foi medido e não é prova". Três exceções, e só elas: decisão do mantenedor, bloqueio ainda aberto, e armadilha que faria o próximo agente refazer o erro (uma linha com a medição, não um parágrafo). Rastro de como se chegou lá vive no histórico do PR, não no doc de estado nem no chat.
...
- **Task executada, bloco atualizado no mesmo turno.** O bloco reflete o que a task **é agora** — estado, medição, bloqueio. Não é adendo: reescreve. Sem narrar commit/push/deploy, que não é fato de task.
...
O que fica aqui é o que **nenhum hook decide** e o que governa **autorização**:
