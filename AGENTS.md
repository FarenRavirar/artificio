...
- **TRAVA PÉTREA — depois de `git push`, o agente para.** Não acompanhar PR, não esperar checks, não rodar `gh pr view`, `gh run watch`, `gh run view`, polling, sleep, nem consultar status do PR após o push, salvo pedido explícito do mantenedor para acompanhar/verificar checks. Se o pedido foi "commit + push", fez push e encerra a resposta.
