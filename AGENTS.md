...
- **SDD Completo:** reservado para mudança grande/arriscada de verdade — infra (Cloudflare Tunnel/DNS), serviço `accounts.` (SSO), migration/banco, permissões, dados pessoais, upload/Cloudinary, importador WP, contrato público/API, deploy, CI/CD, SEO estrutural, ou feature/refator grande.
  Artefatos mínimos: `spec.md` + `plan.md` + `tasks.md` + validação + sessão.

**Mudança em `packages/*` não exige SDD Completo por padrão.** O mantenedor decide caso a caso, na hora, se o escopo pede spec formal ou não — fix pontual/localizado (ex.: ajuste de estilo, correção de bug isolado) segue como Sem SDD/SDD Lite mesmo tocando pacote compartilhado. Só `packages/auth` e o serviço `accounts.` (SSO) mantêm trava própria de aprovação+SDD Completo+smoke (ver §Isolamento de App/Projeto) — é sagrado, não geral.
...
- Matriz mínima de smoke (quando o mantenedor pedir smoke, não trava obrigatória por padrão fora de auth/accounts):
  - `packages/auth` código: login/me/logout e todos os consumidores SSO — este continua obrigatório.
  - `packages/ui`/outros pacotes código: consumidores visuais afetados + app de referência, quando aplicável.
  - `accounts.` código: login/me/logout, allowlist de retorno e pelo menos um app consumidor — este continua obrigatório.
...
- **TRAVA PÉTREA — `git commit --amend` PROIBIDO, sem exceção.** Sempre `git commit` novo, nunca `--amend`. Motivo real: amend reescreve o commit existente sem nova autorização clara por ação, já causou commit sem permissão real do mantenedor e forçou `push --force-with-lease` (reescreve histórico de branch já compartilhada/em review — bots de review e o próprio mantenedor podem perder o rastro do que já foi visto). Se a branch/PR já tem commit anterior e o mantenedor autoriza novo push, sempre `git commit -m "..."` (commit adicional) + `git push` normal (fast-forward, sem `--force`/`--force-with-lease`). Nunca usar `--amend` mesmo se o mantenedor disser "corrige o commit" — nesse caso perguntar se é commit novo (padrão) ou se ele quer literalmente reescrever via outro método explícito.
