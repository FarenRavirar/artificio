...
No primeiro cutover beta, o legado usa o mesmo compose project (`glossario-beta`) com service labels antigas (`app-beta`/`api-beta`/`db-beta`). Isso nao e coberto pelo R1 original da spec 009 (outro project). Por isso `deploy-glossario.yml` liga `reconcile_same_project_orphans` apenas em `dev`: o reusable workflow roda `docker compose down --remove-orphans` no project alvo, sem `-v`, antes do primeiro `up` do DB.
