...
| `glossario` | `glossariobeta.artificiorpg.com` (BETA) / `glossario.artificiorpg.com` (PROD canônico) | 🔄 BETA verde; PROD containers monorepo healthy; DNS público de `glossario.` pendente | ⬜ **spec 012**: corrigir DNS/smoke PROD; `glossariorpg.` era alias histórico desativado; **spec 015**: SSO accounts + compat login antigo (fecha Gate D) |
