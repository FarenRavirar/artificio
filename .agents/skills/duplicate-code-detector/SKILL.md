...
Use parallel task agents, one per duplicate group above the impact threshold, maximum 10 agents per batch. If >10 groups qualify, batch sequentially in groups of 10.
...
Want to start refactoring Priority 1?
```
...
Use **ripgrep (`rg`)** if available (respects `.gitignore` by default, handles nested `.gitignore` files). Use `--type` to scope languages (e.g., `--type ts --type tsx`).

**With ripgrep:**
```bash
# Find most-repeated non-trivial lines, with file locations
rg -n --no-heading --type ts --type tsx '.' /path/to/code \
