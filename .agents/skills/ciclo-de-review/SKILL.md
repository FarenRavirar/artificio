...
**Push era gatilho; nesta conta não é mais.** Medido na PR #328 (2026-09-21,
commit `d74d219`): 7 minutos depois do push, o check veio
`SUCCESS — Review skipped: incremental reviews are disabled`. A revisão
automática por push está **desligada na configuração da conta**, então push
nenhum dispara o CodeRabbit — só o comentário do Passo 3.

Isso inverte a regra que esta seção tinha: o disparo depois do push não é
"desnecessário", é **a única forma**. O que continua valendo é medir antes de
comentar, porque as outras recusas (`rate limited`, `Review skipped: N files`)
não se tratam igual.

A ordem certa depois de um push segue sendo medir o que o bot decidiu:
...
| `SUCCESS — Review skipped: incremental reviews are disabled` | **não revisou**; a conta não faz review por push | **push não é gatilho nesta conta** — só o disparo por comentário revisa. Medido na PR #328 (2026-09-21, commit `d74d219`) |
...
### `commit_id` e `created_at` do comentário NÃO dizem qual commit o bot leu

Duas armadilhas, as duas medidas na PR #328 (2026-09-21):

**`commit_id` é o commit em que o GitHub ANCORA a linha, não o que o bot
revisou.** Um comentário do CodeRabbit veio marcado `d74d219` enquanto o check
da mesma PR dizia `Review skipped: incremental reviews are disabled` — ele não
revisou `d74d219` nenhum. Era achado de `fe13c72` reancorado porque a linha
continuava existindo. Ler o `commit_id` como "revisou este commit" faz a volta
do laço fechar dando código não revisado por aprovado.

**`created_at` vem em UTC e `git log` por padrão não.** O push de `d74d219` saiu
às `23:55:31-03:00`; os comentários diziam `00:10Z` e `00:18Z`. Parecem
posteriores e são **anteriores** — `23:55-03:00` é `02:55Z`. Comparar as duas
strings sem normalizar o fuso inverte a ordem dos eventos. Usar
`git log --format=%cI` e converter, ou comparar tudo em UTC.

O que de fato responde "o bot revisou o HEAD?" é o **check**, não o comentário:
