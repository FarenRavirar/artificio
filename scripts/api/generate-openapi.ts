...
function appendDownloadsQueryParameters(lines: string[], method: string, path: string): boolean {
  ...
  const parameters: Array<{ name: string; description: string; schema: string[] }> = [
    ...
    { name: 'publisher', description: 'Nome ou chave da editora/selo; normalizado no backend para igualdade exata.', schema: ['type: string', 'maxLength: 200'] },
    { name: 'author', description: 'Nome ou chave da autoria; normalizado no backend para igualdade exata.', schema: ['type: string', 'maxLength: 200'] },
